...
from carrybit.config import Config, ModularConfig, load_config
...
def train(cfg: Config, run_dir: Path, device="cuda"):
    ...
    print(f"{cfg.name}: {model.n_params():,} params, {len(task.train[0]):,} training examples")
    ...
    for step in range(cfg.train.steps + 1):
        ...
        if step % cfg.train.eval_every == 0 or last:
            model.eval()
            row = {"step": step, "elapsed": round(time.time() - t0, 1), **task.evaluate(model)}
            model.train()
            if writer is None:
